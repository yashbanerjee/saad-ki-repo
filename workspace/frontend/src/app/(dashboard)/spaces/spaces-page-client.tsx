...
export default function SpacesPageInner() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (created) => {
      ...
      if (created?.id) {
        window.location.href = `/spaces/${created.id}`;
      }
    },
    onError: (err: unknown) => {
      const message =
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={FolderKanban}
          title={tagFilter !== "all" ? "No spaces with this tag" : "No spaces yet"}
          description={
            tagFilter !== "all"
              ? "Try another tag, or clear the filter."
              : "Create your first space and add tags like Vedha, F&S, Web, or ERP."
          }
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/spaces/${project.id}`}>Overview</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/spaces/${project.id}/board`}>Board</Link>
                          </DropdownMenuItem>
                          {canManage && (
                            <>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
                                onClick={() => router.push(`/spaces/${project.id}?edit=1`)}
                              >
